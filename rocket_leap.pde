...
//    enemies.add(new Enemy(mouseX, mouseY, d)); //右クリックで敵追加
  ...
  if(hands.count()>1&&hands.count()<3&&hand[1].palmPosition().getX()<hand[0].palmPosition().getX())
  {
    f=1.0;
  }
  else
...
void drawFingerTip(float a,float b,float d,float e,float f) {
  float fx,fy; //指の位置
  fx = resizeX*a;
  fy = -resizeY*b;
  if(fx<= -w2|| fx>= w2 || fy<= -h2 || fy>= h2 ){
    ...
    if(fy>= h2) y=height;  
    stroke(255);
    drawTriangle(x, y, d);  // 横の位置、縦の位置、円の半径
  }else{
    if(f==1.0){
    text(state1,600,800);
    if(e==1.0&&state1==0)
      ...
      {
        Enemy enemy =new Enemy(n+w2, m+w2, d);
        enemies.add(enemy);
        state1=3;
      }
