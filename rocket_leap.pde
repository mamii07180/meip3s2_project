...
void drawFingerTip(Hand hand) {
  FingerList fingers = hand.fingers();        // FingerList オブジェクトに見つかった指の情報（複数）を入れる
    ...
    if(finger.isExtended() == true) {         // その指が伸びて（isExtended）いたら
      ...
      stroke(255, 0, 0);                    // 塗りつぶし色を白に
    }
    strokeWeight(5);
    line(4*tipPos.getX()+ width/2, 4*tipPos.getY()+8+ height/4, 4*tipPos.getX()+ width/2, 4*tipPos.getY()-8+ height/4);    //撃つ方向
    line(4*tipPos.getX()-16+ width/2, 4*tipPos.getY()+ height/4, 4*tipPos.getX()+8+ width/2,  4*tipPos.getY()+ height/4);    //撃つ方向
  fill(255,0,0);
  textSize(26);
  text(tipPos.getX(), 10, 35);
